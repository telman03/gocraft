...
func setupPerformanceTestDB(t testing.TB) (*gorm.DB, *services.ProjectHistoryService) {
...
func TestCleanupService_Performance(t *testing.T) {
	...
	tempDir := t.TempDir()
	fileService := services.NewFileService(tempDir, 24*time.Hour)
	cleanupService := services.NewFileCleanupService(db, fileService, services.CleanupConfig{
